# Leitura dos registradores: s1 aponta para o disparo atual e s3 para o inimigo testado.
# Os registradores a0-a7 montam os argumentos da colisão; t0-t6 guardam cálculos temporários.
# -----------------------------------------------------------------------------
# Sistema de disparos do mago
# - Tecla ESPAÇO cria um disparo na direção do último movimento (WASD)
# - Até MAX_DISPAROS podem permanecer na tela ao mesmo tempo
...
.eqv MAX_DISPAROS    8    # Limite de disparos simultâneos na tela.
...
# Direção de mira = último movimento do mago (componentes -1/0/1).
...
# na direção de mira atual. Se não houver slot livre, não faz nada.
...
spawn_disp_achou:
    # Posição inicial: centro do mago (24x24) menos metade do disparo (10/2=5)
...
draw_disparo:
    ...
    la   a0, disparo
    addi a0, a0, 8    # Pula o cabeçalho (largura, altura)

    li   t1, 320
    mul  t2, a2, t1
    add  t2, t2, a1
    add  t0, s0, t2    # Endereço do pixel superior esquerdo.
