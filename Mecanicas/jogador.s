# Neste módulo, a0-a3 levam argumentos de desenho ou atualização do jogador.
# Os registradores s preservam ponteiros e contadores durante os laços do HUD.
.data
...
mana_atual:       .word 65    # Valor de exemplo só para visualização.
hud_mana_x:       .word 4
hud_mana_y:       .word 14
...
# Desenha no HUD a quantidade atual de corações do jogador.
draw_hearts:
    addi sp, sp, -16
    sw ra, 12(sp)
    sw s1, 8(sp)
...
# Aplica o dano ao jogador e decide entre reiniciar a posição ou encerrar a partida.
mago_atingido:
    addi sp, sp, -4
    sw ra, 0(sp)
    la t0, escudo_timer
    ...
    la t0, vida_atual
    lw t1, 0(t0)
    addi t1, t1, -1    # t1 passa a guardar uma vida a menos após o contato.
    sw t1, 0(t0)
    blez t1, game_over
...
game_over:
    jal tocar_som_mago_morre    # Na morte, usa um efeito próprio, mais grave e prolongado.
    la t0, game_over_flag    # t0 recebe o endereço da flag lida pelo laço principal.
    li t1, 1
    sw t1, 0(t0)
    lw ra, 0(sp)
    addi sp, sp, 4
